...
- [X] T001 [P] `KnowledgeTree/Localization/Localizable.xcstrings` に新規キーを追加 (約 12 キー):
...
- [X] T002 `KnowledgeTree/Models/Article.swift` を更新する: `@Relationship(deleteRule: .cascade, inverse: \ExtractedKnowledge.article) var extractedKnowledge: ExtractedKnowledge?` を追加。既存 `enrichment` / `body` relationship はそのまま保持
- [X] T003 `KnowledgeTree/KnowledgeTreeApp.swift` を更新する: Schema を `[Article.self, ArticleEnrichment.self, ArticleBody.self]` から `[Article.self, ArticleEnrichment.self, ArticleBody.self, ExtractedKnowledge.self, KeyFact.self, KnowledgeEntity.self]` に拡張。SwiftData lightweight migration で吸収
...
- [X] T004 `KnowledgeTree/Models/ExtractedKnowledge.swift` を作成する。data-model.md / Persistent Types に従い 1 ファイル内に以下を定義:
...
- [X] T019 [P] [US2] `KnowledgeTree/Views/KeyFactRow.swift` を実装する。引数 `KeyFact` を取り、SF Symbol 種別アイコン (event→`calendar`、claim→`bubble.left`、statistic→`chart.bar`、definition→`text.book.closed`、quote→`quote.bubble`) + statement の HStack。`accessibilityIdentifier("knowledgeFactRow")`、`accessibilityLabel` で type + statement を日本語
- [X] T020 [US2] `KnowledgeTree/Views/KnowledgeSummaryView.swift` を実装する。引数 `ExtractedKnowledge` を取り、以下構造:
...
- [X] T021 [US2] `KnowledgeTree/Views/ReaderView.swift` を更新する: 既存 `ScrollView { LazyVStack { ... 本文段落 ... } }` の冒頭に、knowledge .succeeded/.partiallySucceeded のときのみ `KnowledgeSummaryView(knowledge: article.extractedKnowledge!)` を配置 + 「本文」見出し (`Text("knowledge.bodyHeading").font(.title3)`) を本文の前に置く
...
- [X] T022 [P] [US3] `KnowledgeTreeTests/KnowledgeExtractionServiceTests.swift` に availability skip ケースを 3 種追加: (a) `.unavailable(.deviceNotEligible)`、(b) `.unavailable(.appleIntelligenceNotEnabled)`、(c) `.unavailable(.modelNotReady)` のそれぞれで `store.upsertStatus(.skipped)` が呼ばれ、`extractor` (Mock) は呼ばれないこと

### Implementation for User Story 3

- [X] T023 [US3] `KnowledgeTree/Services/KnowledgeExtractionService.swift` の availability チェックを実装で徹底する: `extract(article:)` の冒頭で `SystemLanguageModel.default.availability` を取得し、`.available` 以外なら以下を実行:
