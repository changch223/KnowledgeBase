...
■ 自分好みに調整できる
・本文を自然言語で訂正（「Claude Code を gloadcod と誤認識しています」など入力するだけで AI が該当箇所のみ修正→概念・タグも再生成）
・生成内容をカスタマイズ（「技術的な詳細を重視」「要約を短く」など、まとめ方を指定）
・分類の確認画面でタグの分野を直すと、その正解が次回の自動分類に活かされる
...
Tune it your way
- Fix misrecognized text in natural language ("This should be 'Claude Code', not 'gloadcod'") — AI corrects only that part, then regenerates the concept pages and tags
- Customize what's generated ("emphasize technical detail," "keep summaries short," and more)
- Correct a tag's category and the fix is remembered as a few-shot example for future classifications
