...
> **ja リブランド追記**: 日本語ロケールのみアプリ名を「まとメモ」に変更する提出素材を **§1-L** に追加した
...
## §1-L. ja リブランド: まとメモ（v1.1）

作成日: 2026-07-11 / 対象: ブランチ `rebrand-matomemo-ja`（未 commit）/ ベースコミット `4bd3419`

**背景**: Pocket が 2025-07 にサービス終了し、「あとで読む」難民が発生している。「あとで読む × AI 自動整理」の
日本語検索クエリを正面から取りにいくため、**日本語ロケールのみ** アプリ名を「まとメモ」に変更する。
bundle ID・内部名・型名・CloudKit は一切変更しない（表示名とストア素材のみの変更）。

他 6 ロケール（简体中文 / 繁體中文 / English / 한국어 / Español / Deutsch）の名前は "Knowledge Base" 系のまま
据え置き（§1-J・§1-K がそのまま有効）。以下は **ja ロケールについてのみ** §1-B〜§1-G を上書きする最新版。
...
- 旧: `Knowledge Base：AI第二の脳`（§1-B、21 文字）→ 廃止（ja のみ）。
- 「あとで読む」を名前に直接入れず、代わりに Pocket 難民の検索行動（「記事」「まとめ」「自動」）を直撃する語を採用。
  「あとで読む」自体はサブタイトルで直接使う（L-2）。
...
- 旧: `iPhoneのAIが育てる、第二の脳`（§1-C、18 文字）→ 廃止（ja のみ）。
- 「あとで読む」という Pocket ユーザーに馴染みのある言葉をそのまま検索語として拾いつつ、
  「保存して終わり」ではなく「第二の脳」に変わることを名前直下で伝える。

### L-3. プロモーションテキスト（最大 170 文字・審査なしで随時更新可）
...
- 冒頭を「あとで読む」の痛み（ためたまま読み返せていない）で始める（依頼どおり）。
- 「iPhone の AI を最大限に活かす」「すべて端末内で完結」という §1-D 由来の差別化訴求は維持。

### L-4. キーワード（最大 100 文字・カンマ区切り・スペース禁止）
...
> ASO メモ: 「まとメモ」「メモ」は App 名と重複するためキーワードに入れない（§1-E と同じ方針）。
> 競合商標（Pocket / Instapaper 等）は審査リスクがあるため含めない。「あとで読む」は Pocket 終了後の
> 検索需要を直撃する最重要語としてキーワード先頭かつサブタイトルにも重複配置している（App Store 検索は
> 名前・サブタイトル・キーワードを横断して評価するため、重複配置自体は問題ない）。

### L-5. 説明文（ja）冒頭段落の差し替え

§1-F の説明文は全文書き直し不要。冒頭の「■ 読んだ知識が、勝手に育つ。」段落のみ、以下に差し替える
（それ以外の見出し・本文に "Knowledge Base" という固有名詞の言及はこの段落以外に登場しないため、
残りの段落は §1-F からそのまま流用できる）。
...
- §1-F 側の以降の見出し（■ どこからでも、〜 / ■ AI が、テーマごとに〜 等）はそのまま変更なしで連結する。

### L-6. 「このバージョンの新機能」（What's New / v1.1、ja）

§1-J の J-4（ja What's New v1.1）を ja ロケールに限り以下へ差し替える。末尾に改名の一言を追加した以外は
J-4 の内容（多言語対応・AI 可用性ガイド・AI 復旧の 3 点）をそのまま維持:
...
- §1-I / J-5（App Review 審査メモ）の「共有シートから『Knowledge Base』で保存」という手順文言と、
  ja 端末での実際の表示（「まとメモ」）が食い違う件は **L-8 で対応済み**（審査メモ本体は歴史として
  書き換えず、追記用の 1 文を L-8 に用意した）。
- App Store Connect 側のロケール設定・実際の提出作業（版数決定含む）は本ドキュメント整備の範囲外。

### L-8. 審査メモ追記案（§1-I / J-5 に足す 1 文、日英併記）

§1-I（v1.0）・J-5（v1.1、§1-J）の App Review 審査メモ本体はそのまま変更しない。ja リブランド適用後に
提出する際は、以下の 1 文をどちらの審査メモにも（「【動作要件（重要）】」の直前など任意の位置に）
追記する。日本語ロケールの端末で審査するとアプリ名・共有シートのボタンが「まとメモ」と表示されるため、
既存メモ本文中の "Knowledge Base" という固有名詞表記との食い違いを審査員に事前説明する目的:

```
日本語ロケールの端末ではアプリ表示名・共有シートのボタンが「まとメモ」と表示されます（他言語では
Knowledge Base のままです）。
