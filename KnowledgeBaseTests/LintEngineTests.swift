...
struct LintEngineTests {
    ...
    @Test func testDeleteOrphanedTags() async throws {
        ...
        let tag1 = KnowledgeBase.Tag(name: "OrphanTag")
        let tag2 = KnowledgeBase.Tag(name: "ActiveTag")
        let article = Article(url: "https://example.com", title: "Test")
        article.tags = [tag2]
        context.insert(tag1)
        context.insert(tag2)
        ...
        let remaining = (try? context.fetch(FetchDescriptor<KnowledgeBase.Tag>())) ?? []
        #expect(remaining.count == 1)
    ...
    @Test func testReclassifyBatchIsResumable() async throws {
        ...
        for i in 0..<5 {
            let tag = KnowledgeBase.Tag(name: "tag\(i)")
            context.insert(tag)
        ...
        let tags = try context.fetch(FetchDescriptor<KnowledgeBase.Tag>())
        #expect(tags.count == 5)
    ...
    @Test func testRunBatchResumesFromInterruptedLoop() async throws {
        ...
        let context = container.mainContext
        for i in 0..<4 {
            let tag = KnowledgeBase.Tag(name: "t\(i)")
            context.insert(tag)
    ...
    @Test func testHealConceptsForTagFixesOtherConcept() throws {
        ...
        context.insert(article)
        let tag = KnowledgeBase.Tag(name: "人工知能", categoryRaw: "テクノロジー")  // 分類完了済を模す
        context.insert(tag)
        article.tags = [tag]
