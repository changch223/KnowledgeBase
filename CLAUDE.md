...
- spec 021 — AI Chat (RAG) — 📝 specify+plan 完了 (実装前)。4 タブ目「AI チャット」+ NLEmbedding ベース retrieval (top-k=5) + Foundation Models 回答生成 + 元記事引用 + 会話履歴永続化 (ChatSession / ChatMessage @Model 新規) + Apple Intelligence 不可時 Fallback。
