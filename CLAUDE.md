...
- spec 040 — Knowledge Graph 抽出 + Digest + RAG 統合 (Phase A) — 📝 specify+plan 完了 (本ブランチ、未 commit、2026-05-16)。ユーザー要望「graph で体系化された知識を蓄積、RAG/Digest のデータ元に」。Entity 単位 / ラベル付き+共起ハイブリッド / Category 内 graph / Category 内最大 30 node / 確信度しきい値 0.5+ / UI なし (Phase B で対応)。GraphNode / GraphEdge @Model + GraphExtractionService + GraphTraversalService + 既存 KnowledgeExtractionService/KnowledgeDigestService/ChatService 改修。規模 ~1000-1100 行 / ~18-22 タスク。
- spec 041 — Knowledge Graph UI + 編集 (Phase B) — 📝 specify+plan 完了 (本ブランチ、未 commit、2026-05-16)。spec 040 完了後に着手。Settings の Graph 表示 toggle (default OFF) + CategoryGraphView (SwiftUI Canvas、static 円形 layout) + GraphNodeDetailView + GraphNodeEditSheet (rename/merge/delete、TagStore 同パターン) + GraphEdgeEditSheet + GraphProposalsSection (AI 提案 edge レビュー) + spec 037 統合 (graph triple 衝突)。規模 ~1000-1200 行 / ~20-25 タスク。
