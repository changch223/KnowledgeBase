...
- spec 033 — AI Chat モダン UI 刷新 — 📝 specify+plan 完了 (本ブランチ、未 commit、2026-05-08)。`specs/033-chat-modern-ui/`。NavigationSplitView (iPad sidebar / iPhone overlay) + multi-turn context (直前 4 message) + token streaming + inline 引用 link + session 個別削除 / 新規作成。規模 ~600 行 / ~20-25 タスク。実装前に R1 (Foundation Models streaming API) の実機検証必要。
