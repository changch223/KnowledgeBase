...
struct CategoryStoreTests {
    ...
    @Test func renameCascadesToTagAndConceptCategoryRaw() throws {
        ...
        let tag = KnowledgeTree.Tag(name: "AI", categoryRaw: "テクノロジー")
        ctx.insert(tag)
        let page = ConceptPage(name: "生成AI", categoryRaw: "テクノロジー", summary: "x", updatedAt: .now)
        ctx.insert(page)
        // 別カテゴリの Tag は影響を受けない
        let otherTag = KnowledgeTree.Tag(name: "経済ニュース", categoryRaw: "経済")
        ctx.insert(otherTag)
        try ctx.save()
    ...
    @Test func mergeReassignsCategoryRawAndHidesSource() throws {
        ...
        let tag = KnowledgeTree.Tag(name: "LLM", categoryRaw: "AI")
        ctx.insert(tag)
