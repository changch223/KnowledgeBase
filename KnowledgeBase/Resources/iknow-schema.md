...
- **出力言語は prompt 冒頭の「出力言語」指示に従う** (i18n Phase B)。固有名詞は原語可。
- ページ名そのものの繰り返しを避け、内容で語る。
- **相互リンク** (spec 064): 本文中に参考表の名前が自然に登場したときだけ、その語を `[名前](concept-id://UUID)` 形式のインライン Markdown リンクにする。UUID は参考表の値を一字一句コピーし、創作しない。候補に無い名前にはリンクを付けない。**参考表そのもの・「関連ページ候補」等の見出し・UUID の羅列を本文に出力してはいけない** (リンクは文章中に埋め込む形だけ)。

## 概念階層抽出ルール

spec 074 (概念階層)。記事から「広い概念 1 つ + 具体概念 2-4 個」の 2 階層を抽出するときの指示。

- **広い概念 (broadConcept)**: その記事が属する最も広い概念を 1 つ。分野レベルの代表概念。例: 「生成AI」「データエンジニアリング」「マクロ経済」。広すぎ (例: 「テクノロジー」= カテゴリそのもの) も狭すぎ (具体トピック) も避ける。16 字以内。
- **具体概念 (specificConcepts)**: その広い概念の下で、記事が実際に論じている個別トピックを 2〜4 個。例: 「Text-to-SQL」「RAG」「ワイドテーブル」。各 18 字以内。
- **★体言で書く (最重要)**: 概念名は「短い名詞・専門用語」だけ。**文・説明句・動詞句は禁止**。NG 例:「顧客企業の現場に深く入り込むエンジニア」「金融機関の共同出資による新会社設立」「新領域開拓に集約される可能性があるエンジニア」→ OK 例:「現場常駐エンジニア」「合弁会社」。"〜する/〜による/〜される" を含む説明文になっていたら名詞に削る。
- **明示主義**: 記事に明示されている概念のみ。推測・一般知識での補強は禁止 (source 追跡)。
- **一般語・代名詞・地名を概念にしない**: 「男性」「ユーザー」「企業」「彼女」「東京駅」「湘南」等は不可。
- **重複禁止**: 広い概念と具体概念に同じ名前を入れない。
- **シード優先**: 候補リスト (広い概念のシード) が与えられたら、最も合うものを優先して選ぶ。合うものが無ければ簡潔に命名する。
- **出力言語は prompt 冒頭の「出力言語」指示に従う** (i18n Phase B)。固有名詞は原語可。
